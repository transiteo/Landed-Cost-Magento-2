...
namespace Transiteo\DutiesTaxesCalculator\Model\Config\Backend\Serialized;
    ...
    protected function validateValues(array $array):array
    {
        ...
        foreach ($orderStatusList as $status) {
            if(!in_array($status, $statusList, true)){
                $result[$status] = ["magento_status" => $status, "transiteo_status" => \Transiteo\DutiesTaxesCalculator\Model\Config::TRANSITEO_DEFAULT_STATUS];
            }else{
